...
const create_asset = async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        try {
            ...
            return res.status(400).send({ message: "Invalid JWT token signature or format." });
        }
        ...
        if (user.token_expires_at && new Date(user.token_expires_at) < new Date()) {
            return res.status(401).send({ message: "Your session/token has expired." });
        }
...
const get_asset = async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        // 🎯 The Ownership Check: Filter by BOTH assetId and userId
        const asset = await Asset.findOne({
            where: { 
                id: assetId, 
                user: { id: user.id } 
        ...
        if (!asset) {
            return res.status(404).send({ message: "Asset not found or you do not have access." });
        }
...
const get_all_assets = async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        try { decrypt_Token(token); } catch (e) {
            return res.status(400).send({ message: "Invalid token signature." });
        }
        ...
        const storedToken = user.token ? String(user.token).trim() : null;
        if (!storedToken || storedToken !== sentToken) {
            return res.status(401).send({ message: "Invalid session token mismatch." });
        }
        ...
        if (og_cost && curr_cost) {
            whereConditions.current_cost = Between(Number(og_cost), Number(curr_cost));
        } else if (og_cost) {
...
const assetId = Number(req.params.assetId);
    ...
    try {
        ...
        if (!asset) {
            return res.status(404).send({ message: "Asset not found or you do not have permission to delete it." });
        }
        await asset.remove();
